...
void ActNpc088(NPCHAR *npc)
{
	...
	switch (npc->act_no)
	{
		...
		case 9:
			...
			// Fallthrough
		case 10:
			if (++npc->act_wait > 75 && npc->act_wait % 6 == 1)
			{
				if (npc->direct == 0)
					deg = 0x88;
				else
				...
				deg += (unsigned char)Random(-0x8, 0x8);
				ym = GetSin(deg) * 3;
				xm = GetCos(deg) * 3;
				SetNpChar(11, npc->x, npc->y + (8 * 0x200), xm, ym, 0, NULL, 0x150);

				PlaySoundObject(12, 1);
			}

			if (npc->act_wait > 120 && npc->act_wait / 2 % 2)
				npc->ani_no = 11;
			else
				npc->ani_no = 10;
...
void ActNpc095(NPCHAR *npc)
{
	...
	switch (npc->act_no)
	{
		...
		case 12:
			...
			if (npc->y > npc->tgt_y && npc->act_wait > 5)
			{
				npc->act_wait = 0;
				npc->act_no = 10;
				npc->ani_no = 0;
